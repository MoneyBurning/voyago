...
- morning (오전 도착, ~12시): 오후부터 관광 가능 → 점심 식사 + 오후 일정 2~3개만 포함 (오전 일정 넣지 말 것)
- afternoon (오후 도착, 12~18시): 저녁 일정만 → 저녁 식사 + 야경 명소 1개만 포함 (점심 일정 넣지 말 것)
- evening (저녁 도착, 18시~): 공항→숙소 이동 및 호텔 체크인 항목만 포함 — 식사/관광 등 그 외 일정은 절대 넣지 말 것

## 마지막 날 규칙 — departureTime 기준
- morning (오전 출발, ~12시): 아침 식사 후 바로 공항 이동 → 관광 일정 넣지 말 것
- afternoon (오후 출발, 12~18시): 오전 관광 1~2개 + 점심 식사 후 공항 이동
- evening (저녁 출발, 18시~): 하루 종일 관광 가능 → 아침/점심 포함 풀 일정으로 설계
...
- 위 3끼는 Day 1(도착 시간대별로 위 규칙에 따라 일부/전부 생략 가능)과 마지막 날(출발 시간대별로 위 규칙에 따라 일부/전부 생략 가능)을 제외한 모든 날에 아침/점심/저녁 3끼가 빠짐없이 포함되어야 한다. 최종 JSON을 출력하기 전에 해당하는 모든 날짜에 3끼가 다 있는지 스스로 검토할 것 — 하나라도 빠져 있으면 그 상태로 응답하지 말고 빠진 끼니를 채워 완성한 뒤에 출력할 것
